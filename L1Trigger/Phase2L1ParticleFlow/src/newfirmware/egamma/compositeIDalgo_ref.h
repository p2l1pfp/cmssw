#ifndef COMPOSITEIDALGO_REF_H
#define COMPOSITEIDALGO_REF_H
...
namespace l1ct {
  ...
  struct CompositeIDAlgoEmuConfig {
    unsigned int nTRACK;
    ...
    CompositeIDAlgoEmuConfig(const edm::ParameterSet &iConfig);
    CompositeIDAlgoEmuConfig(unsigned int nTrack,
  ...
  class CompositeIDAlgoEmulator {
  public:
    CompositeIDAlgoEmulator(const CompositeIDAlgoEmuConfig &config) : cfg(config), debug_(cfg.debug) {}

    virtual ~CompositeIDAlgoEmulator() {}
  ...
  private:
    ...
    void comp_algo(const PFRegionEmu &region,
                 const std::vector<EmCaloObjEmu> &emcalo,
                 const std::vector<TkObjEmu> &track,
    ...
    void compute_sumPt(iso_t &sumPt,
                       ...
                       const TEG &egobj,
                       const CompositeIDAlgoEmuConfig::IsoParameters &params,
                       z0_t z0) const {
