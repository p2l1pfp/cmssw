...
private:
  ...
  std::vector<ap_uint<64>> encodeJets(const std::vector<l1t::PFJet> jets, unsigned nJets, l1t::PFJet::HWEncoding encoding);
  ...
  for (const auto& pset : collections_) {
    ...
    if (encodingStr == "CT") {
      ...
      throw cms::Exception("Configuration") 
          << "Invalid jetEncoding value: '" << encodingStr 
          << "'. Valid options are: 'CT', 'GT', 'GTWide'";
    }
...
std::vector<ap_uint<64>> L1CTJetFileWriter::encodeJets(const std::vector<l1t::PFJet> jets, const unsigned nJets, l1t::PFJet::HWEncoding encoding) {
